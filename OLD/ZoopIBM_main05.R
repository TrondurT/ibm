...
options(warn = 2) # If warn is two, all warnings are turned into errors. Default is warn = 0
...
ZMat <- data.frame(ID = forID, # copepod ID
                   ...
                   Age = 1, # total age of copepod
                   TDF = 0) # Thermal development fraction for tracking progress within a stage
...
for (tNow in seq(from = startTime, to = endTime, by = delta.t)){
  ...
  for (st in unique(ZMat$Stage)){ # sort(unique()), can be used to keep the level order
    ...
    ZMat$SDind[indsNow] <- SDinfoNow$SD # add the stage duration info to ZMat
  }
...
ggplot()+
  geom_bar(data = ZMatOut, 
           mapping = aes(x = Time, fill = Stage, position="fill", stat="identity")) # plot abundance by stage over time
